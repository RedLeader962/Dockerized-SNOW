...
- [build the docker image from an x86_64 host using qemu](#build-the-docker-image-from-an x86-host-using-qemu);
...
4. Build the nvidia-docker image
    ```shell
    sudo docker build -t <theImageName>:<theLatestVersionTag> -f Dockerfile .
    ```
   Note:
